"""Tests for Phase 63 - the slash -> dash parcel normalization rule.
...
def enrich(monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location("_p63_enrich", SCRIPT)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["_p63_enrich"] = mod
...
def test_p63_01_slash_form_yields_the_dashed_candidate(enrich, stored, expected):
...
def test_p63_02_as_is_form_is_still_tried_first(enrich):
...
def test_p63_03_parcels_without_a_slash_gain_no_candidate(enrich):
...
def test_p63_04_the_reverse_rule_is_not_added(enrich):
...
def test_p63_05_candidate_list_stays_deduplicated(enrich):
    for stored, _ in CONFIRMED_LIVE:
        candidates = enrich.normalize_candidates(stored)
        assert len(candidates) == len(set(candidates)), stored


def test_p63_06_alnum_only_form_did_not_already_cover_this(enrich):
    ...
    missed under the pre-Phase-63 candidate set."""
