...
**Update (Phase 15, same day): this count was wrong too — same off-by-one this phase has now chased through three separate docs.** A direct count of the literal comma-separated list quoted immediately above (not re-typed, the actual list) is **48**, not 47 — every phase from 14B onward said "47" in prose while writing out a list that has always had 48 names in it, most likely because `id` (the internal UUID, never itself rendered as a value) kept getting excluded from the mental tally despite being included in the literal list every time. Confirmed three independent ways this phase: (1) counting the comma-separated tokens in 005's actual `RETURNS TABLE` clause, live in the repo, returns 48; (2) live production `information_schema.role_column_grants` for `authenticated`/`SELECT` on `public.properties` returns exactly 49 rows, which is 48 (005's output) + 1 (the documented `ledger_type` exception 005a adds) - the two independent counts agree; (3) the CSV-export field reconciliation added this phase (`tests/python/test_phase15_customer_surface_security_audit.py`) passes against a 48-member set. Harmless - no field is missing or extra, only the prose label was off by one - but recorded here rather than left to propagate again. Full account: `docs/phase-15-customer-surface-security-audit.md`.
