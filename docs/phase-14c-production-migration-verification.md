...
## 14. Fresh Gate A re-verification (same day, following Phase 14D)

A second live preflight was performed the same day, directly against production project `cqnnnvpbocafuvpzfbzu`, specifically to re-verify Phase 14D's corrected 005/005a package before ever attempting Gate B. `mcp__Supabase__execute_sql`/`list_edge_functions` succeeded (read access, consistent with this phase's own earlier access). Findings:

- **Column count resolved: 49, not 47.** A direct `information_schema.columns` query for `public.properties` returned exactly 49 rows, matching Section 2's literal column-name list, not its "47" prose claim. `outcome`, `sold_price`, `dor_use_code` are confirmed absent, and no other column present in the live table is missing from that list — the 49-name transcription in Section 2 is accurate; the "47 columns" prose in Section 2 was simply wrong.
- **005's corrected column list checked column-by-column against the live 49.** Every one of the 48 columns 005 (as corrected by Phase 14D) now references exists live, with the single expected exception of `dor_use_code` — confirming the `schema-v9-dor-use-code.sql` → 005 dependency is exact: 005 would fail on that one column alone if run before schema-v9, and would succeed in full immediately after it.
- **Row count**: 3,929 (unchanged from this document's Section 10 baseline). **Identity duplicates**: 0. **Unique constraint**: `properties_state_source_county_case_no_key`, `UNIQUE (state, source, county, case_no)` — all unchanged and re-confirmed live, not assumed.
- **RLS**: `relrowsecurity = true`, `relforcerowsecurity = false`; exactly one policy, `"properties: approved only"`, **PERMISSIVE** (`polpermissive = true`), `cmd = ALL`, `USING`/`WITH CHECK` both `is_approved()` — unchanged from this document's Section 6, re-confirmed live.
- **Grants — broader than previously enumerated.** Table-level grants for `anon`, `authenticated`, and `service_role` on `public.properties` are not just `SELECT`/`INSERT`/`UPDATE`/`DELETE` as this document's Section 5 recorded, but also `REFERENCES`, `TRIGGER`, and **`TRUNCATE`** — i.e., a blanket "all privileges" grant to all three roles, consistent with Supabase's default project-template grant pattern rather than anything this project deliberately configured. `anon`/`authenticated` holding `TRUNCATE` on a live customer table is a real, pre-existing over-grant, independent of the 005/005a question — named here as a new finding, not touched by 005a (which only ever narrows `SELECT`), and not something Phase 14D's hard rules or this phase's own hard rules authorize changing unilaterally. A future phase should review it with its own explicit scope. `information_schema.role_column_grants` was also checked directly (not just the table-level view): all 49 real columns currently show `SELECT` for both `anon` and `authenticated`, confirming no column-level narrowing exists yet (i.e., 005a genuinely has not been run, consistent with everything else this session found).
- **`get_properties()`**: live body is still `select * from public.properties where ...` — the pre-005 definition, `security invoker` (`prosecdef = false`), matching tracked migration 003 exactly. `EXECUTE` granted to `service_role`, `authenticated`, `anon`, `postgres`, and `PUBLIC` — unchanged from this document's Section 5.
- **`digest_candidates()` — refined finding.** Confirmed absent from production again (`information_schema.routines`/`tables`, zero rows). New this pass: `supabase/functions/send-digest/index.ts` *does* call `admin.rpc("digest_candidates", { days_ahead: DIGEST_DAYS })` — so this is not simply an orphaned, uncalled function; a real caller exists in the repository. However, `mcp__Supabase__list_edge_functions` against this project returned **zero deployed Edge Functions** — `send-digest` (and `notify-approval`, `notify-signup`) exist in the repository but none are actually deployed to production, so nothing currently invokes `digest_candidates()` live. Still **NON-BLOCKING DRIFT** for the schema-v9/005/005a chain specifically (unrelated table/function, not referenced by any of the three migrations), but a more precise characterization than "nothing calls it" — worth a future phase's attention if the digest feature is ever meant to ship.
- **Geometry**: re-confirmed no `geom`/`geometry` column exists on `public.properties` (absent from the fresh 49-column list) and exactly the same four non-geometry triggers are attached (`properties_gone_since`, `properties_gone_since_ins`, `properties_touch`, `trg_sync_ledger_type_from_source`) — `properties_sync_geom()` remains defined but orphaned. Unchanged, **RESOLVED / NON-BLOCKING**.
- **Tests**: 151/151 (`tests/python/`), unchanged.

**Gate A = GREEN.** Every item this document's own Section 9 checklist and this phase's fresh instructions required was independently re-verified against live production data this same session, not assumed from any prior phase's snapshot. See `docs/phase-14d-migration-reconciliation.md` for the corrected migration files and `docs/phase-14e-gate-a-execution-attempt.md` for what happened when Gate B (actual execution) was attempted.
