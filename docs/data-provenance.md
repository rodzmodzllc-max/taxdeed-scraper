...
**Status:** Phase 10A (Commercial Source Governance Infrastructure), 2026-09-14, updated Phase 12 (Production Provenance & Data Lineage Integration), Phase 13 (Production Data Contract & Provenance Readiness Gate), and Phase 14A (Production Customer-Safety Hardening), 2026-09-14. Companion to `docs/data-licensing.md` (restriction enforcement), `docs/provenance-production-integration.md` (the full write-up of how the model below is now actually wired into the real harvest/sync pipeline — this page stays the model reference; that page is the integration reference), `docs/production-data-contract.md` (the field-by-field customer-facing contract this lineage model exists to describe — that page maps each real customer-visible column to SOURCE/ENRICHED/DERIVED per this page's own stage vocabulary), and, as of Phase 14A, `docs/phase-14a-customer-safety-hardening.md` (confirms `updated_at`/`gone_since` are real, trigger-maintained production columns — independent of, and in addition to, this page's pipeline-side `Provenance.retrieved_at` — and that no production code reads them back into a customer-facing freshness statement yet). This page is about lineage: for a given value, where did it come from, and what has happened to it since.
