...
`tests/python/test_source_governance.py` — 25 tests, all passing (`pytest tests/python/`, ~0.1s). Covers every scenario Phase 10A Step 12 lists by number (1 through 14), plus additional fail-closed guarantees. See `docs/data-licensing.md`'s "Fail-closed guarantees" section for what each of the extra tests actually protects against.

**Now wired into CI as of Phase 10B** — `.github/workflows/python-governance-test.yml`, triggered on every push/PR to `main`, mirroring the existing `playwright-test.yml` pattern (same triggers, same "a failing test fails the job" contract, no `continue-on-error` or equivalent soft-fail mechanism anywhere in the workflow). This workflow has not yet actually executed on GitHub's runners as of this phase — it was authored and locally validated (YAML parses correctly; the exact `pip install pytest && pytest tests/python/ -v` sequence it runs was reproduced locally and passed) but this sandbox has no push access to `taxdeed-scraper` (see `docs/source-registry.md`/commit-hash notes elsewhere), so the workflow itself will not actually run on GitHub until a human pushes this commit. This is a real, named limitation, not a claim that CI has been "verified working" in the full end-to-end sense.
