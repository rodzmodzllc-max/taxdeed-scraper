...
`tests/python/test_source_governance.py` — 25 tests. `tests/python/test_customer_api_enforcement.py` — 26 tests (Phase 11, added below). 51 tests total, all passing (`pytest tests/python/`, ~0.1s). The first file covers every scenario Phase 10A Step 12 lists by number (1 through 14), plus additional fail-closed guarantees; the second covers every scenario Phase 11 Step 9's own lettered matrix (A through T) lists, plus two extra tests for the field-shape mechanism's inertness against today's real schema and for restriction types that carry no field-shape mapping. See `docs/data-licensing.md`'s "Fail-closed guarantees" section for what each of the Phase 10A extra tests protects against, and `docs/customer-api-data-enforcement.md` for the Phase 11 suite.

**Wired into CI as of Phase 10B, still targeting the whole `tests/python/` directory as of Phase 11** — `.github/workflows/python-governance-test.yml`, triggered on every push/PR to `main`, mirroring the existing `playwright-test.yml` pattern (same triggers, same "a failing test fails the job" contract, no `continue-on-error` or equivalent soft-fail mechanism anywhere in the workflow). Phase 11 added a new test file to `tests/python/` but did not need to touch the workflow itself — `pytest tests/python/ -v` already picks up every file in that directory. This workflow has not yet actually executed on GitHub's runners as of this phase — it was authored and locally validated (YAML parses correctly; the exact `pip install pytest && pytest tests/python/ -v` sequence it runs was reproduced locally and passed, now against 51 tests) but this sandbox has no push access to `taxdeed-scraper` (see `docs/source-registry.md`/commit-hash notes elsewhere), so the workflow itself will not actually run on GitHub until a human pushes these commits. This is a real, named limitation, not a claim that CI has been "verified working" in the full end-to-end sense.
