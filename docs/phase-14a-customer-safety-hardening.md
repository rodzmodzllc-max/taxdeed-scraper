...
| `gone_since` | No (raw value; only feeds derived close-out UI) | Semi | Yes | Yes, indirectly (`GONE_STATUSES`/close-out display) | No | None needed. Confirmed this phase, same way as `updated_at` above, to be a real trigger-maintained column (`track_gone_since`/`track_gone_since_insert` functions appear in the same live advisory output) — not merely a documentation assumption. |
