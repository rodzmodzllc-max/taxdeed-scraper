...
2. **`tx_lgbs` and `tx_realauction` are registered `APPROVED` by production-practice inference, not a formal rights document.** Both are long-shipped, network-verified, unauthenticated public sources with no encountered access wall — but neither has a Phase-8/9.5-style Terms-of-Use/robots.txt/rights-matrix document the way Harris County now does. This is an honest gap, flagged as a backfill-review candidate in `docs/commercial-data-inventory.md`, not a reason this phase held up their continued production operation.
