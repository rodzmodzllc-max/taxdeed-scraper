...
- `harvest_laft_pdfs.py` runs BOTH pdfplumber table strategies (lines and text) on every page and concatenates their tables, so one strategy's header row routinely shows up as a body row of the other's. Until 2026-09-18 that put column headers ("PARCELNUMBER", "IDNUMBER"), footnote fragments and one whole Pasco disclaimer paragraph into `properties.parcel`/`case_no` as if they were properties; `is_plausible_record()` now rejects any parcel cell that is a header label, has no digit, or exceeds `MAX_PARCEL_LEN`. `sync-laft-to-supabase.ps1` has no gone-tracking, so junk that reaches production stays until deleted by hand (`scripts/migrations/013_remove_laft_header_rows.sql`). Don't loosen that gate without a PDF fixture proving the value is a real identifier.
